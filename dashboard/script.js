...
function exibirMensagem(topico, mensagem) {
    try {
        ...
        if(luz && dados.status_luz !== undefined){
            luz.textContent =  dados.status_luz
        }
        ...
        if(movimentoValor && dados.movimentoValor !== undefined){
            movimentoValor.textContent =  dados.movimento ? 'Detectado' : 'Sem movimento'
        }

    } catch (erro) {
        console.erro('A mensagem recebida não é um formato JSON válido:', mensagem)
    }

    if (horarioEl) horarioEl.textContent = "Recebido às " + new Date().toLocaleTimeString("pt-BR");
